...
# load the true.csv.
headlines_true <- read.csv(file.choose(), stringsAsFactors = F)
head(headlines_true)

#Extract Year, Month & Day from Date column
headlines_true$year <- as.numeric(format(as.Date(headlines_true$date, "%d-%b-%y"), "%Y"))
headlines_true$month <- as.numeric(format(as.Date(headlines_true$date, "%d-%b-%y"), "%m"))
headlines_true$day <- as.numeric(format(as.Date(headlines_true$date, "%d-%b-%y"), "%d"))

# count the number of total headlines by date as a table

headlines_true%>% group_by(month) %>% count() %>% arrange(desc(n))
headlines_true %>% group_by(year) %>%count() %>% ggplot() + geom_line(aes(month,n))

table_2016 <- headlines_true %>% 
  filter(year == 2016) %>% 
  group_by(month)  %>%
  count()

table_2017 <- headlines_true %>% 
  filter(year == 2017) %>% 
  group_by(month)  %>%
  count()
...
s <- udpipe_annotate(udmodel_english, headlines_true$title)
x <- data.frame(s)
...
barchart(key ~ freq, data = stats, col = "light blue",
         main = "TRUE - UPOS (Universal Parts of Speech)\n frequency of occurrence",
         xlab = "Freq")
...
barchart(key ~ freq, data = head(stats, 20), col = "cadetblue",
         main = "TRUE - Most occurring nouns", xlab = "Freq")
...
barchart(key ~ freq, data = head(stats, 20), col = "purple",
         main = "TRUE:Most occurring adjectives", xlab = "Freq")
...
barchart(key ~ freq, data = head(stats, 20), col = "gold",
         main = "TRUE - Most occurring Verbs", xlab = "Freq")
...
barchart(key ~ rake, data = head(subset(stats, freq > 3), 20), col = "red",
         main = "TRUE - Keywords identified by RAKE",
         xlab = "Rake")
...
barchart(key ~ freq, data = head(stats, 20), col = "magenta",
         main = "TRUE - Keywords - simple noun phrases", xlab = "Frequency")
...
stats$key <- factor(stats$keyword, levels = rev(stats$keyword))
barchart(key ~ freq, data = head(stats, 25), col = "light blue", main = "TRUE - Keywords - simple
...
ggraph(wordnetwork, layout = "fr") + geom_edge_link(aes(width = cooc, edge_alpha =
  ...
  labs(title = "TRUE -Co-occurrences within 3 words distance", subtitle = "Nouns & Adjectives")
